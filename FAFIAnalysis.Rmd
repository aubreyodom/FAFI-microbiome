...
```{r}
# Read in MAE of data
OG_dat_pre <- readRDS("Data/animalcules_0.5pct.rds")
OG_dat <- subsetByColumn(OG_dat_pre, !colnames(OG_dat_pre) %in% "X3181")
...
# Group 1 <= 16 days
# Group 2 <65 days
pos_grp <- tibble("fAFI(+) Early" = c("0177-1", "0707-1", "0832-1", "1354-1",
                                       rep(NA, 1)),
                  "Neither" = "0058-1",
                  "fAFI(+) Late" = c("0345-1", "0526-1", "0697-1",
                                       "0071-1", "1480-1")) %>%
...
MAE_raw_pre <- readRDS(file.path(stem3, "raw_BEA.RDS"))
MAE_raw <- subsetByColumn(MAE_raw_pre, !colnames(MAE_raw_pre) %in% "X3181")
...
## Read counts
```{R}
all_counts <- colSums(LegATo::parse_MAE_SE(MAE_raw)$counts)
sort(all_counts)
```

```{R}
# Average
median(colSums(counts_table))

sd(colSums(counts_table))/sqrt(ncol(counts_table))
...
init_dat %>%
  ...
  ggpubr::geom_bracket(xmin = "1", xmax = "4", y.position = 40,
    label = "Early Onset Group",
    coord.flip = TRUE) +
  ggpubr::geom_bracket(xmin = "5", xmax = "9", y.position = 90,
    label = "Late Onset Group", coord.flip = TRUE) +
  coord_flip()
...
create_sphaget <- function(this_taxon, tax_level = "genus",
  ...
  prefab_dat %>%
    ggplot2::ggplot(aes(x = Age, y = taxon, group = Subject.ID,
                        color = `Group`)) +
    ggplot2::geom_smooth(mapping = aes(group = `Group`),
                         method = "loess", se = FALSE) +
    ggplot2::geom_line(alpha = 0.4) +
    ggplot2::geom_point(alpha = 0.4, size = 2.5) +
    ggplot2::scale_x_continuous(breaks = seq(0, 120, by = 20)) +
  ...
  file.path(path, paste0("spaghetti_", this_taxon, ".png")) %>%
    ggplot2::ggsave(device = "png", width = 7, height = 3,
                    units = "in")
}
...
conduct_t_test <- function(grp, time, these_samples, counts = counts_lcpm_genus, prefix) {
  ...
  result_pre <- plyr::ldply(all_genera,
...
grp2_t0_samps <- sam_table %>%
  filter(Group == "fAFI(+) Late", Age <= params_mat$end[1]) %>% pull(Sample.ID)
conduct_t_test(2, "t0", grp2_t0_samps,
               prefix = "fullgroup")

grp2_t1_samps <- sam_table %>%
  filter(Group == "fAFI(+) Late", Age > params_mat$start[1 + 1],
         Age <= params_mat$end[1 + 1]) %>% pull(Sample.ID)
conduct_t_test(2, "t1", grp2_t1_samps,
               prefix = "fullgroup")

grp2_t2_samps <- sam_table %>%
  filter(Group == "fAFI(+) Late", Age > params_mat$start[2 + 1],
...
run_beta_test <- function(these_samples) {
  data_in <- subsetByColData(MAE_raw,
                               which(MAE_raw$Sample.ID %in% these_samples))
...
# Obtain raw data
dat_raw <- LegATo::parse_MAE_SE(MAE_raw)

# Extract metadata, taxonomic info, and counts
counts_table_raw <- dat_raw$counts %>%
  select(!all_of(all_neither)) %>%
  upsample_counts(dat_raw$tax, "genus")
...
UMAP4 <- ggplot2::ggplot(aes(x = C1, y = C2, color = Microbe), data = df.umap.m) +
  ggplot2::geom_point(alpha = 0.5, size = 3) +
  paletteer::scale_color_paletteer_d("pals::polychrome", direction = -1) +
  ggplot2::labs(title = "UMAP plot",
...
```{R}
animalcules::diversity_beta_boxplot(MAE_raw, "genus", "bray", "Sepsis.state")
...
# PERMANOVA
PERMANOVA can be done with small sample sizes, but:

You may not get accurate p-values if you can’t generate enough permutations (e.g., < 999 unique permutations).

Power is limited, especially with small effect sizes.

report the number of permutations and check for homogeneity of dispersion (with betadisper)

```{R}
run_permanova <- function(grp, time, fafi_samps, prefix) {
  age_lower <- params_mat %>% filter(timepoint %in% time) %>% pull(start)
  age_upper <- params_mat %>% filter(timepoint %in% time) %>% pull(end)
  controls <- sam_table %>% filter(Sepsis.state == "neg",
                                   Age > min(age_lower),
  ...
  comb_samps <- c(controls, fafi_samps) |> unique()
  
  # Create dfs
  data_matrix <- counts_table |>
    select(all_of(comb_samps)) |>
    t() |> as.data.frame()
  dist_matrix <- vegan::vegdist(data_matrix, method = "bray")
  metadata <- sam_table|> filter(Sample.ID %in% comb_samps)
  
  # Check homoscedasticity 
  vegan::betadisper(dist_matrix, group = as.factor(metadata$Group)) |>
    permutest(permutations = 999)
  
  # Run PERMANOVA using `adonis2` (updated version of `adonis`)
  adonis_result <- vegan::adonis2(dist_matrix ~ Group, data = metadata)
...
grp1_t0_samps <- c("X5264", "X4180", "X12014", "X1998")
run_permanova(grp = 1, time = "t0", fafi_samps = grp1_t0_samps,
              prefix = "fullgroup")

grp2_t0_samps <- sam_table %>%
  filter(Group == "fAFI(+) Late", Age <= params_mat$end[1]) %>% pull(Sample.ID)
run_permanova(2, "t0", grp2_t0_samps,
              prefix = "fullgroup")

grp2_t1_samps <- sam_table %>%
  filter(Group == "fAFI(+) Late", Age > params_mat$start[1 + 1],
         Age <= params_mat$end[1 + 1]) %>% pull(Sample.ID)
run_permanova(2, "t1", grp2_t1_samps,
              prefix = "fullgroup")

grp2_t2_samps <- sam_table %>%
  filter(Group == "fAFI(+) Late", Age > params_mat$start[2 + 1],
         Age <= params_mat$end[2 + 1]) %>% pull(Sample.ID)
run_permanova(2, "t2", grp2_t2_samps,
              prefix = "fullgroup")

grp2_t3_4_samps <- c("X1749", "X4654", "X6731", "X17642", "X7691")
run_permanova(2, c("t3", "t4"), grp2_t3_4_samps,
              prefix = "fullgroup")
