...
```{R}
sam_table %>%
  arrange(Sepsis.state) %>%
  filter(Group != "Neither") %>%
  select(Sepsis.state, Sample, Subject.ID) %>%
  write.csv("Data/Samples_for_Bill.csv")
...
counts_lcpm_genus %>%
  ...
  arrange(desc(V1)) %>%
  right_join(sam_table, by = "Sample")
...
ggsave(filename = "PaperFigs/stacked_barplot_groups_SepsisNeg.png",
...
### Apply function
```{R}
conduct_t_test <- function(grp, time, these_samples, counts = counts_lcpm_genus) {
...
### Species level - Streptococcus
I want to check what type of streptococcus we are seeing in our data at the species level.

```{R}
strep <- counts_lcpm_species %>% rownames() %>%
  stringr::str_split_i("_", 1) %>% is.element("Streptococcus") %>% which()
strep_top <- counts_lcpm_species[strep, ] %>% rowSums() %>% sort(decreasing = TRUE) %>% head() %>% names()
test_strep <- c("Streptococcus_thermophilus",
...
conduct_t_test(2, c("t3", "t4"), grp2_t3_4_samps, counts = strep_counts)
```
...
sample_map <- sam_table %>%
  mutate(Microbe = max_microbe_res,
         ...
         Group == "fAFI(-)" | Group == "Neither",
         "fAFI(-)")) %>%
  select(Sample, all_of(color), `AFI Timepoint`, Microbe, 
...
ggsave(paste0("~/infant-microbiome/work/aodom/Sepsis/PaperFigs/UMAP.png"),
       device = "png", units = "in", width = 6, height = 4)
```
...
ggsave(paste0("~/infant-microbiome/work/aodom/Sepsis/PaperFigs/UMAP_time.png"),
       device = "png", units = "in", width = 6, height = 4)
```
...
ggsave(paste0("~/infant-microbiome/work/aodom/Sepsis/PaperFigs/UMAP_infant.png"),
       device = "png", units = "in", width = 6, height = 4)
```
...
ggsave(paste0("~/infant-microbiome/work/aodom/Sepsis/PaperFigs/UMAP_microbe.png"),
       device = "png", units = "in", width = 6, height = 4)
```
...
this_one <- gridExtra::arrangeGrob(UMAP1, UMAP4, UMAP2, UMAP3,
                                   nrow = 2)
ggsave(paste0("~/infant-microbiome/work/aodom/Sepsis/PaperFigs/UMAP_combined.png"), plot = this_one,
       device = "png", units = "in", width = 10, height = 6)
```
...
ggsave(paste0("~/infant-microbiome/work/aodom/Sepsis/PaperFigs/NMDS_Beta.png"),
       device = "png", units = "in", width = 6, height = 4)
```
